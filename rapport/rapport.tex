...
En C++, comme dans d'autres langages de programmation, des informations telles que le types des paramètres, sont stockées dans nom des fonctions. De manière similaire, nous avons encodé dans le nom des fonctions le types de ses paramètres afin de l'identifier de manière unique. Pour une fonction dont le type des paramètres est fixé, le type de retour est également fixé, donc il n'est pas nécessaire d'ajouter celui-ci au codage.
