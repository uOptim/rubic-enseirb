...
\paragraph{} En Ruby, les structures de contrôles de définissent pas un bloc séparé de la fonction et il est donc nécessaire de repérer les variables introduites dans ces blocs (qui sont d'ailleurs exécutés sous conditions) pour allouer l'espace nécessaire en début de fonction sous peine de créer une erreur de compilation de LLVM à cause d'allocations soumises à condition.
...
Plusieurs fichiers de tests illustrant les fonctionnalités sur compilateur se trouvent dans le répertoire 'tests'. L'ensemble des tests sont compilés sans fuite de mémoire d'après valgrind.
